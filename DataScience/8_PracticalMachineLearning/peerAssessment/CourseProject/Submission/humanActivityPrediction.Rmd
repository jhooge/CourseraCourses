...
```{r}
p1 <- predict(knnModel, na.omit(X_test_selected))
p2 <- predict(naiveBayesModel, na.omit(X_test_selected))
testing <- data.frame(knn=p1, naiveBayes=p2)
testing$knn <- as.character(prediction$knn)
testing$naiveBayes <- as.character(prediction$naiveBayes)
```
